...
window.onload = function () {
    ...
    var hasChild = document.getElementById("girls").hasChildNodes();
    p(hasChild)  //true

  //需求2.获取div中所有子节点
    //先获取id
    var children = document.getElementById("girls").childNodes;
    ...
    var first = document.getElementById("girls").firstChild
    var last = document.getElementById("girls").lastChild
    p(first)
    p(last)
    ...
    var prev = document.getElementById("girls").childNodes[4].previousSibling;
    var next = document.getElementById("girls").childNodes[6].nextSibling;
    p(prev)
    p(prev.innerHTML)
    p(next.innerHTML)
    ...
    var eleNode =document.getElementById("girls")
    p(eleNode)
    ...
    //获得元素id
    ////document.getElementById("girls")表示是整个文本的意思
    var attrNode = document.getElementById("girls").id
    p(attrNode)//打印出来 确认是一个对象

    //nodeName/nodeValue/nodeType
    ////var attrNode = document.getElementById("girls")
    console.info("@@@@@@");         //getAttributeNode通过元素节点的属性名称获取属性的值
    var attrNode = document.getElementById("girls").getAttributeNode("id")
    p(attrNode)  //id="girls"  获取节点内容 //打印出来 确认是一个对象
    p("nodeType="+attrNode.nodeType) //2 常量2代表 属性
    p("nodeName="+attrNode.nodeName)  //id 当前属性名称
    p("nodeValue="+attrNode.nodeValue) //girls 当前属性值

    //文本节点的nodeName/nodeValue/nodeType
    console.info("*****");
    var textNode = document.getElementById("girls").firstChild.firstChild
